...
class SearchLocationViewModel @Inject constructor(
...
) : ViewModel(), ContainerHost<SearchLocationScreenState, SearchLocationScreenSideEffect> {
    override val container: Container<SearchLocationScreenState, SearchLocationScreenSideEffect> =
        container(
            initialState = SearchLocationScreenState(),
            buildSettings = {
                this.exceptionHandler = CoroutineExceptionHandler { _, throwable ->
                    intent {
                        postSideEffect(
                            SearchLocationScreenSideEffect.Toast(
    ...
    companion object {
        val TAG = SearchLocationViewModel::class.simpleName
    }
}

@Immutable
data class SearchLocationScreenState(
    val keyword: String = "",
    val locationUIModelFlow: Flow<PagingData<LocationUIModel>> = emptyFlow(),
    val bookmarkPlaceIdList: List<String> = emptyList()
)

sealed interface SearchLocationScreenSideEffect {
    class Toast(val message: String) : SearchLocationScreenSideEffect
}